...
import { ingest, ingestFunktion } from '../ingest';
...
const plugin: FastifyPluginAsync = async (fastify, opts) => {
	fastify.post('/ingest-funktion', async (request, reply) => {
		...
		if (!file.filename.match(/F\d{4}?-\d{2}\.xlsx/)) {
		...
		let _log = await ingestFunktion(upath);
	...
	fastify.post<{
		...
		for (let file of files) {
			...
			if (file.match(/F\d{4}?-\d{2}\.xlsx/)) {
				_log += await ingestFunktion(dirname('data', file));
			} else {
